...
6. **Instance Type:** Select **`t3.large`** (2 vCPUs, 4GB RAM).

- _Note: If `t3.large` is unavailable in your lab, `t3.medium` will work but may lag with many players._
...
Once you SSH into your instance (`ssh -i your-key.pem ubuntu@your-elastic-ip`), create a bash script file named `game-script.sh` and paste the following command block into the file.
...
Run this "one-liner" script by running `bash game-script.sh` to install everything and launch the games.
